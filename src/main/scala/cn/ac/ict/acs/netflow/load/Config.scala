...
object Config {
  ...
  def getConfigure(filePath: String) = {
    ...
    for (lineStr <- lineIter if !lineStr.startsWith("#") && lineStr.trim.nonEmpty) {
      ...
      key match {
        case "datarate" =>
          dataRate = removeSpecialFlag(cmds(1).trim).toInt * 1024 * 1024 //1MB base
        case "intervalsecond" =>
          intervalSecond = removeSpecialFlag(cmds(1).trim).trim.toInt
        case "hdfspath" =>
          hdfsPath = removeSpecialFlag(cmds(1).trim).trim
        case "startday" =>
          StartTimeInSeconds =
            DateTime.parse(removeSpecialFlag(cmds(1).trim), dhmFormat).getMillis / 1000
        case "endday" =>
          EndTimeInSeconds =
            DateTime.parse(removeSpecialFlag(cmds(1).trim), dhmFormat).getMillis / 1000
        case "rootpath" =>
