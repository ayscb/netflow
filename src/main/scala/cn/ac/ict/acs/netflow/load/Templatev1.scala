...
case class TemplateV1(template: mutable.Map[Int, Int], rowBytes: Int) {
  ...
  private def setFixeLengthData(key: Int, valueLen: Int, row: GenericMutableRow): Unit = {
    valueLen match {
      case 1 =>
        val v = rd.nextInt(TemplateV1.BYTE_MAX).toShort
        row.setShort(key, v)

      case 2 =>
        val v = rd.nextInt(TemplateV1.SHORT_MAX)
        row.setInt(key, v)

      case 4 =>
        val v = Math.abs(rd.nextLong() % TemplateV1.INT_MAX)
        row.setLong(key, v)
    }
  }
...
object TemplateV1 {
   ...
   */
  def load(fileName: String): TemplateV1 = {
